...
# --- 추가 옵션: ANOVA 표시 / 반복 블록요인 / Y축 로그 스케일 ---
show_anova = st.sidebar.checkbox("ANOVA 결과 표시", value=False)
include_rep_block = st.sidebar.checkbox("반복을 블록요인으로 포함(처리 비교)", value=bool(REP_COL)) if REP_COL else False
...
try:
    st.download_button(
        "⬇️ 필터 적용 데이터 CSV",
        data=data_to_use.to_csv(index=False).encode("utf-8-sig"),
        file_name="filtered_voc_data.csv",
        mime="text/csv",
...
if mode in ["처리별 VOC 비교", "시간별 VOC 변화"]:
    if mode == "처리별 VOC 비교":
        ...
        progress_fixed = None
        if compare_mode == "progress 내 treatment 비교" and PROGRESS_COL in data_to_use.columns:
            _pvals = sorted(map(str, data_to_use[PROGRESS_COL].dropna().unique().tolist()))
            default_idx = _pvals.index("treat") if "treat" in _pvals else 0
            progress_fixed = st.sidebar.selectbox("기준 Progress 선택", _pvals, index=default_idx)
        ...
        if selected_interval == "전체":
            data_use = data_to_use.copy()
            title_suffix = "모든 시간"
        else:
            data_use = data_to_use[data_to_use[INTERVAL_COL] == selected_interval].copy()
            title_suffix = f"Interval: {selected_interval}h"

        # progress 내 treatment 비교 시 기준 progress 고정
        title_suffix_extra = ""
        ...
        # 색상 인코딩: treatment 내 progress 비교일 때만 progress로 구분
...
if y_log:
    ...
    # ----- 시간별 VOC 변화 -----
        ...
        # --- 사이드바(시간별 VOC 변화 전용): 처리 선택 ---
        _treat_opts = ["전체"]
        if TREAT_COL in data_to_use.columns:
            _treat_vals = sorted(map(str, data_to_use[TREAT_COL].dropna().unique().tolist()))
            _treat_opts += _treat_vals
        selected_treatment = st.sidebar.selectbox("처리 선택(시계열)", _treat_opts, index=0)
        # -----------------------------------------------
    else:
        # --- 시계열 전용: 처리 선택 UI(없을 경우 생성) ---
        try:
            selected_treatment
        except NameError:
            _treat_opts = ["전체"]
        ...
        if selected_treatment == "전체":
            ...
            data_use = data_to_use[data_to_use[TREAT_COL].astype(str) == str(selected_treatment)].copy()
...
# --- VOC 컬럼 정의 ---
voc_cols = [c for c in data_to_use.columns if c not in [
...
if apply_correction:
    empty_means = (
        data_to_use[data_to_use["Progress"] == "Empty"]
        .groupby(["Treatment", "Repetition"])
        .mean(numeric_only=True)
        .reset_index()
    )
    treat_df = data_to_use[data_to_use["Progress"] == "Treat"].copy()
    merged = treat_df.merge(
        empty_means,
        on=["Treatment", "Repetition"],
        suffixes=("", "_emptymean")
    )
    for col in voc_cols:
        merged[col + "_corrected"] = merged[col] - merged[col + "_emptymean"]
    data_to_use = merged.copy()
else:
