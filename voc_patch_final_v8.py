...
if mode in ["처리별 VOC 비교", "시간별 VOC 변화"]:
    if mode == "처리별 VOC 비교":
        ...
        if compare_mode == "progress 내 treatment 비교" and PROGRESS_COL in filtered_df.columns:
            _pvals = sorted(map(str, filtered_df[PROGRESS_COL].dropna().unique().tolist()))
    ...
    # ----- 시간별 VOC 변화 -----
        ...
        _treat_opts = ["전체"]
        if TREAT_COL in filtered_df.columns:
            _treat_vals = sorted(map(str, filtered_df[TREAT_COL].dropna().unique().tolist()))
